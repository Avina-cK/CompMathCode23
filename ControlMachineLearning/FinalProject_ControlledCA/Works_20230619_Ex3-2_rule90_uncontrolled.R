      ...
      for(i in 2:(N-1)){
        ...
        while (j<=8) {  
          ...
          if(identical(Neighbourhood,Nbs[j,])){ 
            GOL[t+1,i]<-n[j]  # if it is the j-th possible neighbourhood, the j-th rule applies
...
ECA(rule_no, N, T, Rands_0, s_0)->GOL; 
...
title(c(paste("Rule", as.character(rule_no)), "(uncontrolled)"))
