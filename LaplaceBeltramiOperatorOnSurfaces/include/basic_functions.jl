...
Parameterising the surface
 Let x₁ = u and x₃ = v, and x₂ = fₚₛ(u,v)
 S = {(u,v) : fₚₛ(u,v) = 0}
    where fₚₛ(u,v) = ± sqrt(1-(v²)-((u-v²)²))
=#

"""
dₛ(x): distance function that defines the surface
    S = {x ∈ ℜ³ : (x₁ - x₃²)² + x₂² + x₃² = 1}
"""
function dₛ(vp::Gridap.Point)
    x1 = vp[1];
    x2 = vp[2];
    x3 = vp[3];
    dist_S = ((x1 - (x3*x3))^2) + (x2*x2) + (x3*x3) - 1.0;
    return dist_S
end
...
function ∂₁n₁(vp::Gridap.Point)
    ...
    denom = η_val^(-3/2);
    num11 = -2.0 * x3 * x3
    num12 = x1 + (2.0 * x2 * x2) + (2.0 * x3 * x3) - 2.0
    num = (num11 * num12) + 1.0;
    return denom * num;
end
...
"""
grad_n₁(x::Gridap.Point) 
"""
function grad_n1(vp::Gridap.Point)
    gn1 = ∂₁n₁(vp);
    gn2 = ∂₂n₁(vp);
    gn3 = ∂₃n₁(vp);
    return Gridap.Point(gn1, gn2, gn3);
end
...
grad_n₂(x::Gridap.Point) 
"""
function grad_n2(vp::Gridap.Point)
    gn1 = ∂₁n₂(vp);
    gn2 = ∂₂n₂(vp);
    gn3 = ∂₃n₂(vp);
    return Gridap.Point(gn1, gn2, gn3);
...
grad_n₃(x::Gridap.Point) 
"""
function grad_n3(vp::Gridap.Point)
    gn1 = ∂₁n₃(vp);
    gn2 = ∂₂n₃(vp);
    gn3 = ∂₃n₃(vp);
    return Gridap.Point(gn1, gn2, gn3);
...
"""
div_n(x::Gridap.Point)
"""
function div_n(vp::Gridap.Point)
    d1 = ∂₁n₁(vp);
    d2 = ∂₂n₂(vp);
    d3 = ∂₃n₃(vp);
    return d1 + d2 + d3
...
function true_u(vp::Gridap.Point)
    return vp[1]*vp[2]
end
function ∇u_true(vp::Gridap.Point)
    ∇u₁ = vp[2];
    ∇u₂ = vp[1];
    ∇u₃ = 0.0;
    final_∇u_true = Gridap.Point(∇u₁,∇u₂,∇u₃);
...
# ∇vⱼ = 
function grad_v_f_j(vp::Gridap.Point, j::Int)
    x1 = vp[1];
    x2 = vp[2];
    x3 = vp[3]; 
    n₁ = n1(vp);
    ...
    if j==1
        p1 = Gridap.Point(-n₁*n₂, 1.0 - (n₁*n₁), 0.0);
        s1 = (2.0 * x2 * n₁) + (x1 * n₂);
        s2 = x1*n₁
        final_grad_v_f_j = p1 - (s1 * grad_n₁) - (s2 * grad_n₂)
    elseif  j==2
        p1 = Gridap.Point(1.0 - (n₂*n₂),-1.0*n₁*n₂,  0.0);
        s1 = n₂*x2;
        s2 = (n₁*x2) + (2.0*n₂*x1);
        final_grad_v_f_j = p1 - (s1 * grad_n₁) - (s2 * grad_n₂)
    elseif j==3
...
# ∇⋅v = -(∇(∇u ⋅ n)⋅n) - (∇u ⋅ n)(∇⋅n)
function div_v_f(vp::Gridap.Point)
    gradu = ∇u_true(vp);
    normal = n(vp);
    graddot = vp[1]*grad_n2(vp) + vp[2]*grad_n1(vp) + Gridap.Point(n2(vp), n1(vp), 0.0);
...
#f = -∇⋅v + Σ₁³(∇vⱼ⋅n)nⱼ
function rhs_f(vp::Gridap.Point)
    div_v = div_v_f(vp);
    normal = n(vp);
