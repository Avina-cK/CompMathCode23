...
function Sim_ODE_SchnakenbergSys_64_extL(Du::Float64, Dv::Float64, a::Float64, b::Float64, U0::Matrix{Float64}, V0::Matrix{Float64}, nsteps::Int64=Int64(10000);
                                    solvername = RK4(), deltat::Float64= 0.001, saveat_t::Float64 = 0.1, togif::Bool=true, SaveUVf::Bool=false, colourgrad = les7, plotwidth::Int=1000, extra_iter::Int=1)
    nx = size(U0, 1);
    ny = size(V0, 1);  
    N = nx
    # ------------- defining ODE --------------------------
    ...
    prob_ode_schnakenberg_2d = ODEProblem(Schnakenberg_2d_ODE, u0, (0.0, final_T), p)
    ...
    trajectory = solve(prob_ode_schnakenberg_2d, solvername, saveat = saveat_t, save_on = save_every, dt=deltat, progress=true);
    ...
    function frameV(V, step, whichvar; subsize = (plotwidth, 800))
            ...
            else plottitle = L"\textit{v(T)}, " *"       "* "time ="*" $(round(step,digits=2)) \n " * "for "*L"[D_{u}, D_{v}, a, b] = " * "[$(params)]"
            end
...
extra_iter_val = 1;
# -- Simulation ----------------------------------------------
Sim_ODE_SchnakenbergSys_64_extL(Dᵤ, Dᵥ, a, b, U0, V0, nsteps
